...
#' @param sig_mat The covariance matrix of the test statistics (assume multivariate normal).
#' @param muVec The mean vector of the test statistics (assume multivariate normal).
#'
...
#' power_from_bounds(bounds = myBounds, sig_mat = myVariance, muVec = c(1, 0, 0, 0, 0))
#'
power_from_bounds <- function(bounds, sig_mat, muVec) {
  ...
  calc_over_s <- function(s, p, sig_mat_a, mu_a, Delta_p_part) {
    ...
    integral_result <- pmvnorm(lower=lower_bounds, upper=upper_bounds, mean=T_mu, sigma=T_var_matrix)
  ...
  pvalue <- 1 - sum(sumOverA[1, ])
  errsum <- sum(sumOverA[2, ])
  naSum <- sum(sumOverA[3, ])
  
  return(list(pvalue=pvalue, errsum=errsum, sumOverA = sumOverA, naSum = naSum))
}