...
#' sim_R2c(n=5000, muVec = c(1, 0, 0, 0, 0), sigMat = myVariance, bounds=myBounds)
#'
sim_R2c<- function(n, muVec, sigMat, bounds) {
  ...
  checkBoundsR2c <- function(x, bounds) {
    newBounds <- bounds
  ...
  crossVec <- apply(zSample, 1, checkBoundsR2c)
